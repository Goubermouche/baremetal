...
function main() {
	...
	instructions.forEach(inst => {
		// if(inst.category.includes('GP') && !inst.extension.includes('X86') && ['RM', 'MR', 'M', 'NORMAL', 'R', 'VEX', 'VEX_VM', 'VEX_RM', 'VEX_RVM'].includes(inst.enc.enc) && inst.name === 'xor') {
		if(inst.category.includes('GP') && !inst.extension.includes('X86') && ['VEX', 'VEX_VM', 'VEX_RM', 'VEX_RVM'].includes(inst.enc.enc)) {
			gp_inst.push(inst);
		}
	});
