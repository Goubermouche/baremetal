...
namespace baremetal {
	module::module() : m_current_inst_begin(0) {}

	auto module::get_bytes() const -> const utility::dynamic_array<u8>& {
		return m_bytes;
	}

	auto module::find_instruction_info(u32 index, const operand* operands)  -> const instruction* {
	...
	auto module::is_legal_variant(u32 a, u32 b, u8 imm_index) -> bool {
	...
	void module::emit_opcode_prefix_vex(const instruction* inst, const operand* operands) {
	...
	auto module::get_instruction_l(const instruction* inst) -> bool {
		if(inst->is_l1()) {
			return true;
		}
...
void module::emit_opcode_prefix_vex_two(const instruction* inst, const operand* operands) {
		// two byte vex prefix
	...
	auto module::get_instruction_vvvv(const instruction* inst, const operand* operands) -> u8 {
		switch(inst->enc) {
			// VEX
	...
	void module::emit_opcode_prefix_vex_three(const instruction* inst, const operand* operands) {
		if(inst->is_xop()) {
	...
	void module::emit_opcode_prefix_evex(const instruction* inst, const operand* operands) {
		...
		switch(inst->enc) {
			...
			case ENC_EVEX_RVM: {
				if(inst->operand_count == 2) {
					...
					vvvv = static_cast<u8>((~operands[1].r & 0b00001111) << 3);
				}
	...
	void module::emit_opcode_prefix_rex(const instruction* inst, const operand* operands) {
	...
	void module::emit_instruction_prefix(const instruction* inst) {
	...
	void module::emit_instruction_opcode(const instruction* inst, const operand* operands) {
	...
	void module::emit_opcode_prefix_rex_mem(const mem& memory) {
		// extended base
	...
	void module::emit_operands(const instruction* inst, const operand* operands) {
		for(u8 i = 0; i < inst->operand_count; ++i) {
	...
	void module::emit_instruction_mod_rm(const instruction* inst, const operand* operands) {
		u8 rx = m_regs[0];
	...
	void module::emit_instruction_sib(const operand* operands) {
		operand operand;
	...
	void module::emit_instruction(u32 index, const operand* operands) {
	...
	auto module::has_sib_byte(const operand* operands) -> bool {
		operand operand;
	...
	auto module::get_mod_rm_reg(const instruction* inst, const operand* operands) -> u8 {
		u8 rx = m_regs[0];
	...
	auto module::get_instruction_rex_rex(const instruction* inst, const operand* operands) -> u8 {
		const bool is_rexw = inst->is_rexw();
	...
	auto module::get_instruction_rex_evex(const instruction* inst, const operand* operands) -> u8 {
		const bool is_rexw = inst->is_rexw();
	...
	auto module::get_instruction_rex_vex(const instruction* inst, const operand* operands) -> u8 {
		const bool is_rexw = inst->is_rexw();
	...
	auto module::get_instruction_rex(const instruction* inst, const operand* operands) -> u8 {
		const u8 operand_count = inst->operand_count;
	...
	void module::emit_data_operand(u64 data, u16 bit_width) {
		for(u16 i = 0; i < bit_width / 8; ++i) {
			m_bytes.push_back(data >> (i * 8) & 0xFF);
		}
	}

	auto module::sib(u8 scale, u8 index, u8 base) -> u8 {
	...
	auto module::rex(bool w, u8 rx, u8 base, u8 index) -> u8 {
	...
	auto module::mod_rx_rm(u8 mod, u8 rx, u8 rm) -> u8 {
	...
	auto module::direct(u8 rx, u8 reg) -> u8 {
		return mod_rx_rm(DIRECT, rx, reg);
	}

	auto module::indirect(u8 rx, u8 base) -> u8 {
		return mod_rx_rm(INDIRECT, rx, base);
	}

	auto module::indirect_disp_8(u8 rx, u8 base) -> u8 {
		return mod_rx_rm(INDIRECT_DISP8, rx, base);
	}

	auto module::indirect_disp_32(u8 rx, u8 base) -> u8 {
		return mod_rx_rm(INDIRECT_DISP32, rx, base);
	}

	void module::instruction_begin(const instruction* inst, const operand* operands) {
	...
	auto module::get_current_inst_size() const -> u8 {
